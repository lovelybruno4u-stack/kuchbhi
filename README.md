...
# Optional for platforms that break multiline JSON:
...
## Troubleshooting (Render / Invalid JWT Signature)

If you see `invalid_grant: Invalid JWT Signature`, usually the service-account key format is broken in env vars.

1. Preferred: upload JSON file and set `GOOGLE_CREDENTIALS_FILE` path.
2. If using env var JSON, make sure `private_key` preserves newlines (`\n`).
3. Best on Render: set `GOOGLE_SERVICE_ACCOUNT_JSON_B64` with base64-encoded JSON.
4. Regenerate key in Google Cloud IAM if key may be corrupted/revoked.
5. Ensure service account has access to the sheet (or set `GOOGLE_OWNER_EMAIL` and let app auto-create/share).
